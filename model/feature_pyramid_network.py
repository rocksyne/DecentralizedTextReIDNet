...
class FeaturePyramidNetwork(nn.Module):
    def __init__(self, out_channels_per_stage_level:List = [24, 80, 192, 1280], feature_size: int = 256) -> list:
        """
        ...
        self.conv3x3 = DepthwiseSeparableConv(in_channels=feature_size,
                                                out_channels=feature_size,
                                                kernel_size=3,
                                                bias=True)

        self.P_10_conv = DepthwiseSeparableConv(in_channels=S9_channels,
                                                  out_channels=feature_size,
                                                  kernel_size=3,
                                                  bias=True)
        
    def forward(self, stage_outputs_as_inputs:List[torch.Tensor]):

        if not isinstance(stage_outputs_as_inputs, list):
            raise TypeError("stage_outputs_as_input must be a list and contain 4 elements.")

        S3_output, S5_output, S7_output, S9_output = stage_outputs_as_inputs
