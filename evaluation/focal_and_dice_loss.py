...
class FocalAndDiceLoss(nn.Module):
    def __init__(self, use_sigmoid: bool = True, gamma: float = 2.0, alpha: float = 0.25, loss_weight: float = 1.0):
    ...
    def forward(self,cate_preds, kernel_preds, ins_pred, gt_bbox_list, gt_label_list, gt_mask_list):
        return self.loss(cate_preds, kernel_preds, ins_pred, gt_bbox_list, gt_label_list, gt_mask_list)
    ...
    def get_seg(self, cate_preds, kernel_preds, seg_pred, img_metas, cfg, rescale=None):
        ...
        for img_id in range(len(img_metas)):
            ...
            kernel_pred_list = [
                kernel_preds[i][img_id].permute(
                    1, 2, 0).view(-1, self.kernel_out_channels).detach()
