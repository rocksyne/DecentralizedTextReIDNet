        ...
        with tqdm(train_set_loader, unit='batch') as train_set_loader_progress:
            ...
            for train_data_batch in train_set_loader_progress:
                ...
                with torch.autocast(device_type=config.device,  dtype=precision_dtype):
                    aa = model(images,  human_gt_bboxes, human_gt_labels, human_gt_masks)

                    # loss_inputs = human_outs + (human_mask_feat_pred, human_gt_bboxes, human_gt_labels, human_gt_masks)

                    # # calculate loss
                    # hp_loss = human_parsing_loss(*loss_inputs)
