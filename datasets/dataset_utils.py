...
def get_ids_from_MHPv2_txt_file(text_file_path:str=None):
	"""
	...
	if text_file_path is None:
		raise ValueError("`text_file_path` parameter is invalid. Provided path is: ", text_file_path)

	if os.path.exists(text_file_path) is False:
		raise ValueError("`text_file_path` does not exist in storage. Provided path is: ", text_file_path)
