...
def receive_message(socket, data_queue: queue.Queue, shutdown_event):
...
def confidence_reporting_thread(client, camera_name, confidence_queue, shutdown_event):
...
def process_video(data_queue, camera_stream, confidence_queue, model, configs, camera_name, top_1, shutdown_event, sender):
                ...
                if textual_embedding is not None:
                    ...
                    if persons_embeddings_out is not None:
                        ...
                        if filtered_boxes:
                            ...
                            sorted_boxes = [sorted_boxes[i] for i in sorted_indices]
...
def health_check(master_address, shutdown_event, sender):
    """Health check to terminate if master node is unreachable."""
    while not shutdown_event.is_set():
        try:
            # Ping master node or perform a simple connectivity check
            sock = socket.create_connection((master_address, 1883), timeout=10)
            sock.close()
            time.sleep(5)  # Check every 5 seconds
        except (socket.timeout, socket.error):
            print("Master node unreachable, shutting down.")
            shutdown_event.set()  # Trigger shutdown
            break
        # Check if the sender is connected
        if not sender.socket.getsockopt(zmq.EVENTS) & zmq.POLLOUT:
            print("Image sender disconnected, attempting to reconnect.")
            try:
                sender = reconnect_sender(sender)
            except Exception as e:
                print(f"Failed to reconnect sender: {e}")
                shutdown_event.set()
                break

def reconnect_sender(sender):
    """Attempts to reconnect the ImageSender."""
    sender.close()
    sender = ImageSender(connect_to=sender.connect_to)
    return sender
...
if __name__ == "__main__":
    ...
    recv_thread = threading.Thread(target=thread_target_wrapper(receive_message), args=(subscriber_socket, data_queue, shutdown_event))
    recv_thread.daemon = True
    recv_thread.start()

    video_thread = threading.Thread(target=process_video, args=(data_queue, camera_stream, confidence_queue, model, configs, camera_name, top_1, shutdown_event, sender))
    video_thread.daemon = True
    video_thread.start()

    reporting_thread = threading.Thread(target=confidence_reporting_thread, args=(mqtt_client, camera_name, confidence_queue, shutdown_event))
    reporting_thread.daemon = True
    reporting_thread.start()

    health_check_thread = threading.Thread(target=health_check, args=(configs.command_station_address, shutdown_event, sender))
    ...
    try:
        ...
        print("All threads and resources have been cleaned up.")
