...
def log_bandwidth(data_size, camera_count):
    global data_received, start_time, bandwidth_log
    ...
    if elapsed_time >= 1.0:  # Log every second
        bandwidth_log.append((current_time, data_received, camera_count))
        print(f"Time: {current_time}, Data Received: {data_received / (1024 * 1024):.2f} MB, Camera Count: {camera_count}")
        data_received = 0
        start_time = current_time

def log_frame_rate(camera_id):
    global camera_frame_count, camera_start_time

    if camera_id not in camera_frame_count:
        camera_frame_count[camera_id] = 0
        camera_start_time[camera_id] = datetime.now()

    camera_frame_count[camera_id] += 1
    elapsed_time = (datetime.now() - camera_start_time[camera_id]).total_seconds()

    if elapsed_time >= 1.0:  # Log every second
        frame_rate = camera_frame_count[camera_id] / elapsed_time
        print(f"Camera: {camera_id}, Frame Rate: {frame_rate:.2f} FPS")
        camera_frame_count[camera_id] = 0
        camera_start_time[camera_id] = datetime.now()
...
def receive_video(bind_ip, bind_port):
    ...
    try:
        ...
        while True:
            ...
            # Calculate the size of the received frame
            data_size = frame.nbytes
            camera_count = len(frames) + 1  # Number of active cameras
            log_bandwidth(data_size, camera_count)

            # Log frame rate for each camera
            log_frame_rate(camera_id)
